...
A comprehensive Bash script to automate the maintenance of Debian-based Linux systems. This tool ensures your environment is always updated, upgraded, and cleared of unnecessary package clutter.
...
The script utilizes both `apt upgrade` and `apt full-upgrade`. While a standard upgrade only installs newer versions of existing packages, a **full-upgrade** is more robust—it can remove packages if necessary to resolve dependencies for a complete system refresh.
